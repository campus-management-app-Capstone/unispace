...
/**
 * Build the survey cards that are visible for the current signed-in role.
 */
function getVisibleSurveyCardItems(role: string | undefined): SurveyCardItem[] {
  return surveyCardItems.filter((surveyCardItem) => {
    if (surveyCardItem.isStudentOnly) return role === "student";
...
/**
 * Protected survey page that shows role-appropriate feedback forms.
 */
export default async function SurveyPage() {
  const user = await currentUser();
  const userRole = (user?.publicMetadata?.role as string | undefined) ?? undefined;
  const visibleSurveyCardItems = getVisibleSurveyCardItems(userRole);
  ...
  return (
    <section className="space-y-6">
      <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">
            Choose a survey below to open the corresponding Google Form that is
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        {visibleSurveyCardItems.map((surveyCardItem) => {
